...
class User {
   ...
   */
  get privateChat() {
    return this.client.cache.chats.find(
      (chat) => chat.users.size === 1 && chat.users.first().id === this.id
    );
  }
  ...
  _patch(data) {
     ...
     */
    this.isVerified =
      "is_verified" in data ? data.is_verified : this.isVerified;
    /**
     * @type {boolean}
     * Whether the user is a business profile
     */
    this.isBusiness =
      "is_business" in data ? data.is_business : this.isBusiness;
    /**
     * @type {string}
     * The URL of the user's avatar
     */
    this.avatarURL =
      "profile_pic_url" in data ? data.profile_pic_url : this.avatarURL;
    /**
     * @type {string?}
     ...
     */
    this.mediaCount =
      "media_count" in data ? data.media_count : this.mediaCount;
    /**
     * @type {number?}
     * The number of followers of the user
     */
    this.followerCount =
      "follower_count" in data ? data.follower_count : this.followerCount;
    /**
     * @type {number?}
     * The number of followed users by the user
     */
    this.followingCount =
      "following_count" in data ? data.following_count : this.followingCount;
    /**
     * @type {number?}
     * The number of videos published by IGTV
     */
    this.totalIgtvVideos =
      "total_igtv_videos" in data
  ...
  async fetchFollowers() {
    if (!this.followersFeed)
      this.followersFeed = this.client.ig.feed.accountFollowers(this.id);
  ...
  async fetchFollowing() {
    if (!this.followingFeed)
      this.followingFeed = this.client.ig.feed.accountFollowing(this.id);
