const { withRealtime, withFbns } = require("node_instagram_mqtt");
const {
  GraphQLSubscriptions,
  SkywalkerSubscriptions,
} = require("node_instagram_mqtt/dist/realtime/subscriptions");
...
class Client extends EventEmitter {
  ...
  /**
   * Fetch and format messages from a chat.
   * @param {string} threadId The ID of the chat/thread to fetch messages from.
   * @returns {Promise<Array<Object>>} An array of formatted messages.
   */
  async fetchMessages(threadId) {
    ...
    const reversedThreadItems = threadItems.reverse();

    const formattedThreadItems = reversedThreadItems.map((item) => {
      const role = item.is_sent_by_viewer === true ? "assistant" : "user";
      return { role, content: item.text };
    });

    return formattedThreadItems;
  }
  /**
